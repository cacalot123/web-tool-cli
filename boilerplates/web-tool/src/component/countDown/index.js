...
const hoc = WrappedComponent => class extends Component {
  constructor(props) {
    ...
    const { endTime, second, timeoutFn } = this.props;
    if (!endTime && !second) { //应传入这两个参数中一个
    ...
    if (timeoutFn !== undefined && typeof timeoutFn !== 'function') {
  ...
  render() {
    ...
    if (totalSecond > 0) {
      ...
      const countdownTime = this.props.second ? `${s}秒` : ({
        day: d,
        hour: h,
        minute: m,
        second: s
      ...
      const props = {
        ...
        hasPassTime: this.state.hasPassTime,
        countdownTime,
        ...this.props
      };
      return (
