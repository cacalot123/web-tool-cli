...
const formValidation = {
  isEmpty(val, errorMsg) {
    if (val === '') {
      return errorMsg;
    }
  },
  hasSpace(val, errorMsg) {
    if (/\s/g.test(val)) {
      return errorMsg;
    }
  },
  ...
  maxLength(val, length, errorMsg) {
    if (val.length > length) {
      return errorMsg;
    }
  },
  mustLetterHead(val, errorMsg) {
    if (!/^[a-z]/i.test(val)) {
      return errorMsg;
    }
  },
  isNumberHead(val, errorMsg) {
    if (/^\d/.test(val)) {
      return errorMsg;
    }
  },
  isZeroHead(val, errorMsg) {
    if (/^0/.test(val)) {
      return errorMsg;
    }
  },
  mustAllNum(val, errorMsg) {
    if (/\D/g.test(val)) {
      return errorMsg;
    }
  },
  isPhoneNum(val, errorMsg) {
  ...
  isID(val, errorMsg) {
    const ary = val.split('');
    const facAry = [7, 9, 10, 5, 8, 4, 2, 1, 6, 3, 7, 9, 10, 5, 8, 4, 2];
    let tempFac = 0;
    for (let i = 0, l = facAry.length; i < l; i++) {
  ...
  isBirth(val, errorMsg) {
    const dateArr = val.match(/\d+/g);
    const date = dateInfo(new Date(dateArr.join()));
    ...
    function isVali() {
      const newDate = dateInfo(new Date());
...
export default class Validator {
  ...
  valiAllByDom() {
    for (const name in this.domRuleCache) {
      this.valiOneByDom(name);
    }
  }

  valiALlByValue() {
    for (const name in this.valueRuleCache) {
      this.valiOneBy(name);
    }
  }
